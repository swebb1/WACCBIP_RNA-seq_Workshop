...
We will use RStudio in the second part of this workshop to analyses read count data. RStudio can also be used to view the files we create on the server in a genome browser. Check that you can login at [http://aabn.ug.edu.gh/auth-sign-in](https://eur02.safelinks.protection.outlook.com/?url=http%3A%2F%2Faabn.ug.edu.gh%2Fauth-sign-in&data=05%7C02%7C%7C308be624c262423dbeb408dc256eb9c4%7C2e9f06b016694589878910a06934dc61%7C0%7C0%7C638426405753131265%7CUnknown%7CTWFpbGZsb3d8eyJWIjoiMC4wLjAwMDAiLCJQIjoiV2luMzIiLCJBTiI6Ik1haWwiLCJXVCI6Mn0%3D%7C0%7C%7C%7C&sdata=SmKwPGIJq35pVVqzWUFEkDYwQ6XtvRqjaGUQEXflpwA%3D&reserved=0 "Original URL: http://aabn.ug.edu.gh/auth-sign-in. Click or tap if you trust this link.")
...
Unix has a program called **parallel** which allows you to run tools on multiple datasets at the same time. The following command lists all of your gzipped fastq files and pipes "\|" them into parallel.
...
-   *ps* is the *process status* tool which shows jobs running in the current session, we should see 6 instances of fastqc running.
...
First, let's create a file that lists our sample names so we can feed this into our pipeline. We could just type this manually or use a spreadsheet and export it as a tab-separated file. We will use a bit of unix text processing, don't worry too much if you don't understand the command. For this exercise we will only use one of the replicates.

``` bash
ls fastq/*_1.fq.gz | parallel basename | sed s/.fq.gz// > samples.txt
```
...
**Emacs** is a Unix text editor. Here we are going to paste all of the commands we have used so far:
    ...
    ls /data/swebb/training/RNA-seq_analysis/lesson_1/fastq/*fq.gz | parallel -j 6 ln -s {} fastq/
...
``` bash
cat samples.txt | parallel -j 2 "fastqc fastq/{}.fq.gz"
```
:::
...
cat samples.txt | parallel -j 2 "trim_galore --fastqc -q 20 --illumina --length 20 --cores 4 -o trim_galore fastq/{}.fq.gz"
```
:::
...
cat samples.txt | parallel -j 2 "STAR --genomeDir /data/swebb/training/RNA-seq_analysis/annotation_small/STAR_index_hg38_chr1.ensembl106 --readFilesCommand zcat --readFilesIn trim_galore/{}_trimmed.fq.gz --runThreadN 2 --outSAMtype BAM SortedByCoordinate --outFileNamePrefix STAR/{}. --sjdbGTFfile annotation/Homo_sapiens.GRCh38.106.gtf --outWigType wiggle --outWigNorm RPM --genomeLoadLoadAndRemove --limitBAMsortRAM 30000000000"
```
:::
...
::: blue
``` bash
cat samples.txt | parallel -j 2 "samtools index STAR/{}.Aligned.sortedByCoord.out.bam" ## Index the bam files
```
:::
...
::: blue
``` bash
cat samples.txt | parallel -j 2 "samtools view -F 260 -bh -o  STAR/{}.primary.bam STAR/{}.Aligned.sortedByCoord.out.bam; samtools index STAR/{}.primary.bam"
```
:::
...
::: blue
``` bash
cat samples.txt | parallel -j 2 "samtools view -b -q 255 STAR/{}.primary.bam -o STAR/{}.unique.bam; samtools index STAR/{}.unique.bam"
```
:::
...
cat samples.txt | parallel -j 2 "wigToBigWig STAR/{}.Signal.Unique.str1.out.wig annotation/Homo_sapiens.GRCh38.dna.primary_assembly.len visualisation/{}.unique.r.bw; wigToBigWig STAR/{}.Signal.Unique.str2.out.wig annotation/Homo_sapiens.GRCh38.dna.primary_assembly.len visualisation/{}.unique.f.bw"
```
:::
...
cat samples.txt | parallel -j 2 "salmon quant -i annotation/hg38.cdna.ncrna.salmon.index -l SR -r trim_galore/{}_trimmed.fq.gz -p 5 --seqBias --gcBias -o salmon/{}"
```
:::
...
mkdir fastq
## Link the raw data to the fastq folder
ls /library/training/RNA-seq_analysis/data/lesson_1/fastq/*fq.gz | parallel -j 2 ln -s {} fastq/
## Link the annotation folder
...
## index STAR
cat samples.txt | parallel -j 2 "samtools index STAR/{}.Aligned.sortedByCoord.out.bam"
